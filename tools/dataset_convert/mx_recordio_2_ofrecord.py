...
def main(args):
    ...
    output_file = os.path.join(output_dir, "part-{:05d}")
    
    f = None
    record_index = -1
    approx_bytes = 0
    record_size_limit = 64 * 1024 * 1024 # 64 MB    
    for idx in imgidx_list:
        if f is None:
            record_index += 1 
            f = open(output_file.format(record_index), "wb")
        if approx_bytes > record_size_limit:
            approx_bytes = 0
            f.close()
            record_index += 1
            f = open(output_file.format(record_index), "wb")
        ...
        header, s = recordio.unpack(rec)
        img_data["label"] = int(header.label)
